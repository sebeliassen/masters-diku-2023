...
class QSAGEConv(SAGEConv):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        if l_eq_r:
            ...
            ###
            # if config.last_epoch:
            #     os.makedirs("simulated_act", exist_ok=True)

            #     # Generate a random 6-digit number
            #     random_number = int(torch.randint(100000, 1000000, (1,)).item())

            #     # Save the tensor to a file
            #     file_path = os.path.join("simulated_act", f'tensor_{random_number}.pt')
            #     torch.save(dim_reduced_input, file_path)
            ### 
