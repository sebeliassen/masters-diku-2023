...
from exact.utils import empty_cache, get_memory_usage, compute_tensor_bytes, swap_to_cpu, cast_low_bit_int, scale_middle
...
def quantize_and_pack(data, bits, mn, mx):
    if config.simulate:
        ...
        if config.use_optimal_lo:
            # opt_dict = {16: 1.05, 63: 1.086, 32: 1.069, 76: 1.09}
            dim = data.size(-1)
            # if dim in opt_dict.keys():
            #     config.lo = opt_dict[dim]
...
def no_scheme_compute_quantization_bits(input):
    ...
    if config.perc != 0.0:
        lo, hi = F.estimate_quantiles(input_flatten, num_quantiles=2, offset=config.perc)
        input_flatten = torch.clamp(input_flatten, min=lo, max=hi)
...
def quantize_activation(input, scheme):
    ...
    # hardcoded for now
    if input.size(-1) == 63:
        config.col_size == 63 * 4
    elif input.size(-1) == 32:
        config.col_size == 32 * 8
    else:
        config.col_size == input.size(-1)
    ...
    # if config.lo != 1.0:
    #     input_groups = scale_middle(input_groups, q_min, mx, config.lo)
